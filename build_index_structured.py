...
INDEX_NAME = f"{config.FQ_SCHEMA}.chunks_structured_index"
SOURCE_TABLE = f"{config.FQ_SCHEMA}.chunks_structured"

existing = [i["name"] for i in vsc.list_indexes(config.VS_ENDPOINT).get("vector_indexes", [])]
if INDEX_NAME in existing:
    print(f"Index '{INDEX_NAME}' already exists.")
else:
    print(f"Creating index '{INDEX_NAME}' and embedding 96 chunks...")
    kwargs = dict(
